                                ...
                                <table id="table" class="table table-bordered dt-responsive nowrap table-striped align-middle" style="width:100%">
                                    ...
                                    <tbody>
                                        @foreach ($customers as $customer)
                                            @php
                                                $verify = App\Models\CustomerContribution::isExist($customer->id,$contribution->id)
                                            @endphp
                                            <tr>
                                                <td>
                                                    <input style="cursor:pointer" {{$verify===true ? 'checked' : ''}} class="form-check-input selected" type="checkbox" data-id="{{ $contribution->id }}" data-customer-id="{{ $customer->id }}">
                                                </td>
...
@section('script')
    <script>
        ...
        $(".selected").on('click',function(){
            ...
            $.ajax({
                type: 'GET',
                url: '{{route("contribution.set_set_customer")}}',
                data: {
                    contribution_id:$(this).data('id'),
